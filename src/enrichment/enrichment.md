...
1. Read normalized Greenhouse jobs from PostgreSQL (`green_job`).
2. Select only rows where `candidate IS NULL`.
3. Apply the assignment regex filter using job title and company name.
4. Persist `candidate = TRUE` or `candidate = FALSE` back to `green_job`.
5. For rows classified `candidate = TRUE`, insert a matching `job_id` into `green_enrich` if no row exists yet.
6. Pull individual Greenhouse job detail responses for candidate jobs as needed.
7. As soon as a job-detail GET returns a checked HTTP response, mark `green_job.enriched = TRUE`.
8. If the response is `404`, record `green_enrich.request_status = 404`, leave the other enrichment fields blank, and let later downstream handling decide what to do with the gone job.
9. Use `green_enrich` as the job-level enrichment store for later ranking inputs.
...
- `green_enrich`
  ...
  - Current columns:
    - `job_id` (primary key and foreign key to `green_job.job_id`)
    - `description`
    - `min_salary`
    - `max_salary`
    - `currency`
...
- The candidate filter operates incrementally by reading only `green_job` rows with `candidate IS NULL`.
...
- `green_enrich` stores the latest job-level enrichment state, not enrichment history.
- Candidate processing must be idempotent:
  - already-classified jobs are skipped
  - existing `green_enrich` rows are not modified during candidate classification
...
- A checked `404` is treated as a terminal availability result for the current enrichment pass and is stored in `green_enrich.request_status`.

## candidate_filter.py Behavior

- Connects to PostgreSQL using the shared env loader pattern used elsewhere in the repo.
- Reads unresolved jobs from `green_job`.
...
- Inserts `job_id` into `green_enrich` only for rows classified `TRUE`, and only when that `job_id` is not already present.
...
- Reads work from `green_enrich` joined to `green_job`.
- Selects rows that are present in `green_enrich` and still have `green_job.enriched = FALSE`.
...
- On successful `200` responses, updates `green_enrich`, writes `request_status = 200`, and marks `green_job.enriched = TRUE`.
- On checked `404` responses, writes `green_enrich.request_status = 404`, marks `green_job.enriched = TRUE`, and leaves the other enrichment fields blank.
- Leaves non-404 request failures eligible for retry by keeping `green_job.enriched = FALSE`.

## utility/pull_ten_jobs.py Behavior

- Pulls sample individual Greenhouse job responses for local enrichment context.
- Selects random jobs from `green_job` where `candidate = TRUE`.
- Uses `token` and `greenhouse_job_id` to request the individual Greenhouse job endpoint with `pay_transparency=true` and `questions=true`.
...
- Preserve `green_job` as the thin normalized source table and store heavier downstream fields in enrichment-specific tables.
